...
test("rejects catalog entries whose source is missing", () => {
  ...
  fs.writeFileSync(path.join(root, "catalog/plugins.json"), JSON.stringify({ schemaVersion: "0.1", plugins: [{ id: "missing-plugin", path: "plugins/missing-plugin", category: "official", runtime: "isolated-worker", integrationKind: "self-authored-media" }] }));
...
test("rejects catalog entries with an SDK-invalid manifest", () => {
  ...
  fs.writeFileSync(path.join(root, "catalog/plugins.json"), JSON.stringify({ schemaVersion: "0.1", plugins: [{ id: "invalid-plugin", path: "plugins/invalid-plugin", category: "adapter", runtime: "isolated-worker", integrationKind: "local-service-bridge" }] }));
...
test("rejects a symlinked catalog source", (t) => {
  ...
  fs.writeFileSync(path.join(root, "catalog/plugins.json"), JSON.stringify({ schemaVersion: "0.1", plugins: [{ id: "linked-plugin", path: "plugins/linked-plugin", category: "official", runtime: "isolated-worker", integrationKind: "self-authored-media" }] }));
