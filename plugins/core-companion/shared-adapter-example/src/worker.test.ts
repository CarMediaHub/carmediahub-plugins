...
test("shared adapter exposes only public context and logical routes", async () => {
  ...
  for (const locale of ["en", "zh-CN", "ko"] as const) {
    assert.deepEqual(await handler!({ method: "GET", path: "/health", context: { ...client.context, locale } }, new AbortController().signal), { status: 200, body: { status: "ok", worker: "shared-adapter-example", locale, policyVersion: 3 } });
  }
  assert.deepEqual(await handler!({ method: "GET", path: "/health" }, new AbortController().signal), { status: 200, body: { status: "ok", worker: "shared-adapter-example", locale: "en", policyVersion: 0 } });
