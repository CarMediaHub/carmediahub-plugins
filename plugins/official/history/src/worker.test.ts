...
test("history plugin delegates scoped query, record and clear to Core", async () => {
  ...
  const client = { context, close() {}, history: () => ({
    query: async () => [{ id: "h1" }],
    queryPage: async (input: unknown) => { calls.push({ method: "queryPage", input }); return { entries: [{ id: "h1" }], total: 1 }; },
  ...
  assert.deepEqual(await handler!({ method: "GET", path: "/", query: { keyword: "road", category: "video", limit: "2" } }), { status: 200, body: { entries: [{ id: "h1" }], total: 1 } });
  ...
  assert.deepEqual(calls, [
    { method: "queryPage", input: { keyword: "road", category: "video", limit: 2, offset: 0 } },
...
test("history plugin rejects malformed entries and unknown routes", async () => {
  let handler: ((request: GatewayWorkerRequest) => Promise<unknown>) | undefined;
  const client = { context, close() {}, history: () => ({ query: async () => [], queryPage: async () => ({ entries: [], total: 0 }), record: async () => { throw new Error("unused"); }, clear: async () => 0 }), onGatewayRequest(registered: (request: GatewayWorkerRequest) => Promise<unknown>) { handler = registered; } } as unknown as WorkerClient;
  const worker = await startHistoryWorker({ endpoint: "local", installationId: "history", runtimeCredential: "credential" }, async () => client);
...
test("history UI uses the same-origin logical route and safe text rendering", () => {
  ...
  assert.match(html, /Number\.isSafeInteger\(total\)/u);
