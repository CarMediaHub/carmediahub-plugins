...
test("WDR worker exposes only its initial logical health and entry routes", async () => {
  ...
  const records = new Map<string, { key: string; value: unknown; updatedAt: string }>();
  const data = { get: async <T>(_collection: string, key: string) => records.get(key) as { key: string; value: T; updatedAt: string } | undefined, put: async <T>(_collection: string, key: string, value: T) => { const record = { key, value, updatedAt: new Date().toISOString() }; records.set(key, record); return record; }, delete: async (_collection: string, key: string) => records.delete(key), list: async <T>() => [...records.values()] as { key: string; value: T; updatedAt: string }[], migrate: async () => ({ version: 1, name: "test", appliedAt: new Date().toISOString() }), migrateBatch: async () => [{ version: 1, name: "test", appliedAt: new Date().toISOString() }], migrations: async () => [] };
  const client: WorkerClient = { context: { scope: context.scope, locale: "en", timeZone: "UTC", theme: "system", density: "comfortable", entry: "navigation", display: context.display, policyVersion: 1 }, close: () => { closed = true; }, database: () => data, call: async <T>() => ({ media: [] } as T), jobs: () => ({ enqueue: async () => { throw new Error("not used"); }, list: async () => [], cancel: async () => undefined }), history: () => ({ record: async () => { throw new Error("not used"); }, query: async () => [], queryPage: async () => ({ entries: [], total: 0 }), clear: async () => 0 }), catalog: () => ({ register: async () => { throw new Error("not used"); }, query: async () => [], remove: async () => false }), display: () => ({ capabilities: () => context.display, requestMode: async () => ({ mode: "normal" as const, accepted: true }) }), media: () => ({ createPlayback: async () => { playbackSessions += 1; return { sessionId: "playback_test", mediaId: "clip-1", expiresAt: new Date(Date.now() + 1000).toISOString() }; }, probe: async () => ({ mediaId: "clip-1", contentType: "video/mp4", size: 4, updatedAt: new Date().toISOString(), seekable: true, availableModes: ["direct-range" as const], recommendedMode: "direct-range" as const }), requestTransform: async () => { throw new Error("not used"); }, readOutput: async () => ({ data: "", completed: true, contentType: "video/mp4", size: 0 }), requestHls: async () => { throw new Error("not used"); }, readHlsAsset: async () => ({ data: "", completed: true, contentType: "application/vnd.apple.mpegurl", size: 0 }) }), network: () => ({ request: async () => { throw new Error("not used"); } }), browser: () => ({ request: async () => { throw new Error("not used"); }, list: async () => [], revoke: async () => false, enqueue: async () => { throw new Error("not used"); }, tasks: async () => [], cancelTask: async () => undefined }), notifications: () => ({ publish: async () => { throw new Error("not used"); }, list: async () => [], markRead: async () => false, markAllRead: async () => 0 }), onGatewayRequest: (registered) => { handler = registered; }, onContextChanged: () => () => undefined };
  ...
  assert.deepEqual(await handler!({ method: "GET", path: "/recent" }, signal), { status: 200, body: { items: [] } });
  assert.deepEqual(await handler!({ method: "POST", path: "/progress", body: { mediaId: "clip-1", positionSeconds: 12, durationSeconds: 120 } }, signal), { status: 200, body: { saved: true } });
  assert.equal((await handler!({ method: "GET", path: "/recent" }, signal) as { body: { items: Array<{ positionSeconds: number }> } }).body.items[0]?.positionSeconds, 12);
  assert.deepEqual(await handler!({ method: "POST", path: "/progress", body: { mediaId: "not-found", positionSeconds: 1 } }, signal), { status: 404, body: { code: "CMH.WDR.MEDIA_NOT_FOUND" } });
