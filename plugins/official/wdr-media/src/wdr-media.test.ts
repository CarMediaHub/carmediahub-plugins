...
test("WDR worker exposes only its initial logical health and entry routes", async () => {
  ...
  let closed = false;
  const client: WorkerClient = { context: { scope: context.scope, locale: "en", timeZone: "UTC", theme: "system", density: "comfortable", entry: "navigation", display: context.display, policyVersion: 1 }, close: () => { closed = true; }, call: async <T>() => ({ media: [] } as T), jobs: () => ({ enqueue: async () => { throw new Error("not used"); }, list: async () => [], cancel: async () => undefined }), history: () => ({ record: async () => { throw new Error("not used"); }, query: async () => [], clear: async () => 0 }), catalog: () => ({ register: async () => { throw new Error("not used"); }, query: async () => [], remove: async () => false }), display: () => ({ capabilities: () => context.display, requestMode: async () => ({ mode: "normal" as const, accepted: true }) }), onGatewayRequest: (registered) => { handler = registered; }, onContextChanged: () => undefined };
